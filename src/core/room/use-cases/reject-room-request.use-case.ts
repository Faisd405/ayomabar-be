...
@Injectable()
export class RejectRoomRequestUseCase {
  ...
  async execute(hostUserId: number, requestId: number) {
    ...
    if (roomRequest.room.userId !== hostUserId) {
      throw new ForbiddenException('Only the room host can reject requests');
    }

    // Check if room is public (public rooms auto-accept, no manual rejection needed)
    if (roomRequest.room.roomType === 'public') {
      throw new BadRequestException('Public rooms do not require manual approval/rejection');
    }

    // Check if request is already processed
    if (roomRequest.status === 'accepted') {
      throw new BadRequestException('Cannot reject an already accepted request');
    }
